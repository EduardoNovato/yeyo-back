...
class ProveedorBase(BaseModel):
    ...
    #id_proveedor: int = Field(..., description="ID del proveedor", primary_key=True)
    ...
    num_art_comprados: int = Field(0, description="Número de artículos comprados")
    num_art_devoluciones: int = Field(0, description="Número de artículos devueltos")
    num_art_vendidas: int = Field(0, description="Número de artículos vendidos")
    num_compras: int = Field(0, description="Número de compras realizadas")
    valor_comprado: Decimal = Field(Decimal(0.00), description="Valor total comprado")
    num_devoluciones: int = Field(0, description="Número de devoluciones realizadas")
    valor_devuelto: Decimal = Field(Decimal(0.00), description="Valor total devuelto")
    num_ventas: int = Field(0, description="Número de ventas realizadas")
    valor_vendido: Decimal = Field(Decimal(0.00), description="Valor total vendido")
    rating_art_provedor: Optional[Decimal] = Field(Decimal(0.00), description="Rating del proveedor basado en artículos")
